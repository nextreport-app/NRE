...
| `AUTH_GOOGLE_ID` / `AUTH_GOOGLE_SECRET` | Only if using "Continue with Google" | From a Google Cloud Console OAuth client. In that client's **Authorized redirect URIs**, add `https://<your-domain>/api/auth/callback/google` for **every** domain the app is served on (production domain and any Vercel preview/`.vercel.app` URL still in use) — a domain missing here is the most common cause of Google sign-in failing right after the account picker. Leave blank to disable Google login (email/password still works). |
