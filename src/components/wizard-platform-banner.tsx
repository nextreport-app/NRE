...
export function WizardPlatformSummaryLabel({ platform }: { platform: Platform }) {
  return <span className="text-[14px] text-white">{getPlatformLabel(platform)}</span>;
}
...
export function WizardGoogleGenerateBanner() {
  return (
    <div className="rounded-lg border border-dash-border bg-dash-card p-4">
      <p className="text-[14px] text-dash-ink-secondary">All campaigns in your file are included — generate when ready.</p>
    </div>
  );
}
