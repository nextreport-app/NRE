...
describe("renderPptx — real template end-to-end", () => {
  ...
  it("grows the table past its native 10 columns for 3+ simultaneous objectives — Fix 1, no objective dropped, verified with an independent OOXML reader", async () => {
    ...
    const reachRows = buildDailyRows({
      ...
      reach: 10000,
      impressions: 15000,
      // Non-zero, unlike other Reach fixtures in this file — this test's own
      // point is 3 SIMULTANEOUS objective columns, so this one must be a
      // real (non-zero-count) objective too, or the zero-count-column fix
      // (round 4) drops it and there are only 2 objectives to grow from.
      results: 5,
      link_clicks: 0,
      ctr: 0.8,
      cpc: 0,
