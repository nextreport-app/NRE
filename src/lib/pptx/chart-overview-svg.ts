...
export function buildMtdOverviewSvg(chart: ShareChartData): string {
  ...
  const parts: string[] = [
    ...
    `<text x="${MTD_VISUAL.leftX}" y="${MTD_VISUAL.panelY + 14}" fill="${MUTED}" font-family="Poppins" font-size="14" font-weight="700">${escapeXml(model.leftHeading)}</text>`,
    `<text x="${MTD_VISUAL.rightX}" y="${MTD_VISUAL.panelY + 14}" fill="${MUTED}" font-family="Poppins" font-size="14" font-weight="700">${escapeXml(model.rightHeading.toUpperCase())}</text>`,
  ];
  ...
  if (model.isMultiObjective && model.groupedDonut) {
    ...
    parts.push(
      `<text x="${x + d / 2}" y="${y + d / 2}" text-anchor="middle" fill="${INK}" font-family="Poppins" font-size="16" font-weight="700">${escapeXml(model.groupedDonutCenterLabel)}</text>`,
    );
  } else {
  ...
  parts.push(
    `<text x="${MTD_SLIDE_W / 2}" y="${MTD_VISUAL.summaryY + 18}" text-anchor="middle" fill="${MUTED}" font-family="Poppins" font-size="14">${escapeXml(model.summaryLine)}</text>`,
    "</svg>",
  );
  return parts.join("");
}
