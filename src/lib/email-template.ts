...
export function buildReportEmailHtml(props: ReportEmailProps): string {
  const branding = props.reportBranding ?? buildReportBrandingSettings({ agencyName: props.agencyName });
  const header = emailBrandingHeader(branding);
  ...
  const generatedBy = escapeHtml(emailGeneratedByLine(branding, props.senderName));
  const footerPrimary = emailFooterPrimary(branding, props.senderName, props.agencyName);
  const footerSite = emailFooterSiteLine(branding);
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
            <tr>
              <td style="padding-bottom:28px;text-align:center;">
                ${
                  header.showNextReportWordmark
                    ? `<span style="font-size:20px;font-weight:700;color:${TEXT};letter-spacing:-0.02em;">Next<span style="color:${ACCENT};">Report</span></span>`
                    : `<span style="font-size:20px;font-weight:700;color:${TEXT};letter-spacing:-0.02em;">${escapeHtml(header.subtitle)}</span>`
                }
                <p style="margin:6px 0 0;font-size:13px;color:${TEXT_MUTED};">${header.showNextReportWordmark ? "Performance Report" : "Client performance report"}</p>
              </td>
            </tr>
            <tr>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${BORDER};border-radius:8px;">
                  <tr>
                    <td style="padding:16px 20px;font-size:13px;line-height:1.9;color:${TEXT_MUTED};">
                      ...
                      <strong style="color:${TEXT};">Generated by:</strong> ${generatedBy}
                    </td>
                  </tr>
                </table>
              </td>
            ...
            <tr>
              <td style="padding-top:20px;text-align:center;">
                ${
                  footerPrimary
                    ? `<p style="margin:0 0 4px;font-size:12px;color:${TEXT_MUTED};">${escapeHtml(footerPrimary)}</p>`
                    : ""
                }
                ${
                  footerSite
                    ? `<p style="margin:0 0 12px;font-size:12px;color:${TEXT_MUTED};">${escapeHtml(footerSite)}</p>`
                    : `<p style="margin:0 0 12px;font-size:12px;color:${TEXT_MUTED};">&nbsp;</p>`
                }
...
export function buildReportEmailText(props: ReportEmailProps): string {
  ...
  lines.push(
    ...
    `Generated by: ${emailGeneratedByLine(branding, props.senderName)}`,
  );
  const footerPrimary = emailFooterPrimary(branding, props.senderName, props.agencyName);
  const footerSite = emailFooterSiteLine(branding);
  if (footerPrimary) lines.push("", footerPrimary);
  if (footerSite) lines.push(footerSite);
  return lines.join("\n");
}
