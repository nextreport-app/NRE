...
/** Streams the signed-in user's agency logo for Account settings preview. */
export async function GET() {
  ...
  try {
    const user = await prisma.user.findUnique({
      ...
      select: { agencyLogoUrl: true },
    });
    if (!user?.agencyLogoUrl) return NextResponse.json({ error: "No logo set." }, { status: 404 });
    ...
    return apiErrorResponse(err, "account:logo:get");
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const logoUrl = await saveAgencyLogo(session.user.id, processed.buffer, processed.format);
    await prisma.user.update({ where: { id: session.user.id }, data: { agencyLogoUrl: logoUrl } });
    if (user?.agencyLogoUrl && user.agencyLogoUrl !== logoUrl) {
      await deleteLogoFile(user.agencyLogoUrl).catch(() => {});
    }
    return NextResponse.json({ ok: true });
  } catch (err) {
    return apiErrorResponse(err, "account:logo:upload");
  }
}
...
export async function DELETE() {
  ...
  try {
    ...
    return apiErrorResponse(err, "account:logo:delete");
  }
}
